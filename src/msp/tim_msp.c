...
void HAL_TIM_Encoder_MspInit(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM2) {
        ...
        // Enable interrupts
        HAL_NVIC_SetPriority(EXTI2_IRQn, ENCODER_RESET_IRQ_PRIORITY, ENCODER_RESET_IRQ_SUBPRIORITY);
        HAL_NVIC_SetPriority(TIM2_IRQn, ENCODER_TIM_IRQ_PRIORITY, ENCODER_TIM_IRQ_SUBPRIORITY);
